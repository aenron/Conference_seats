...
@mcp.tool(name="query_personnel_candidates", description="根据姓名、部门、单位、行政职务或专业技术职务查询参会人员候选名单。candidates 仅返回 name、department、organization、professional_title、management_post_level（聘任管理岗位等级）、administrative_title 六项会议排位所需信息；不返回工号等个人信息。所有数组字段传 [] 而不能传 null。")
